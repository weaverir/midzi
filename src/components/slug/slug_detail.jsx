                            ...
                            <div className="grid justify-center gap-3 px-3 grid-cols-3 md:grid-cols-3 lg:grid-cols-4">
                                {product.variants.map((variant) => variant.color)
                                    ...
                                    .map((color, index) => {
                                        ...
                                        const discountForColor = product.variants
                                            .filter((variant) => variant.color.name === color.name)
                                            .some((variant) => variant.discount_percentage > 0);
                                        return (
                                            <div
                                                key={index}
                                            ...
                                            >
                                                <div
                                                    className={`rounded-tr-xl relative rounded-tl-xl border-t-2 border-l-2 border-r-2 h-10 w-20 ${
                                                ...
                                                >
                                                    {discountForColor && (
                                                        <span className="text-white  rounded-full font-semibold font-sans w-[18px] flex justify-center top-1 items-center h-[18px] bg-red-500 right-[55px]   absolute  text-[15px] ">%</span>
                                                    )}
                                                </div>
                                                <div
                            ...
                            <div className="grid justify-center gap-3 px-3 grid-cols-3 md:grid-cols-3 lg:grid-cols-4">
                                {availableSizes.map((size, index) => {
                                    const sizeOutOfStock = product.variants
                                        ...
                                        .every((variant) => variant.stock === 0);
                                    const discountForSize = product.variants
                                        .filter((variant) => variant.size.name === size.name && variant.color.name === selectedColor.name)
                                        .some((variant) => variant.discount_percentage > 0);
                                    return (
                                        <div
                                            key={index}
                                            className={`rounded-xl relative justify-center p-4 text-sm dark:bg-bgdark dark:text-text_w items-center flex bg-white h-10 w-20 border-2 cursor-pointer ${
