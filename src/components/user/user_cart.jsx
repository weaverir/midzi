...
const UserCart = () => {
    ...
    const calculateTotalPrice = (basketItems) => {
        ...
        return totalPrice >= 2000000 ? totalPrice : totalPrice + 65000; // Free shipping for total price >= 2000000
    };
    ...
    const handleOrderSubmit = async () => {
        ...
        const url = paymentMethod === 'reserve'
            ? `https://midzi.liara.run/accounts/basket/${basketId}/request/reserved/`
            : `https://midzi.liara.run/accounts/basket/${basketId}/request/` ;
        try {
            const response = await axiosInstance.get(
                url,
                {
                    params: { address_id : selectedAddress },
                    headers: {
                ...
                {basketItems.length > 0 && (
                    <>
                        <div className="mt-4 flex flex-col md:flex-row justify بین items-center gap-4">
                            <p className="text-lg font-semibold text-black dark:text-text_w">تعداد کل
                                اقلام: {calculateTotalCount(basketItems)}</p>
                            <p className="text-lg font-semibold text-black dark:text-text_w">
                                جمع کل: {calculateTotalPrice(basketItems).toLocaleString()} تومان
                                {calculateTotalPrice(basketItems) >= 2000000 ? '(ارسال رایگان)' : '(به همراه 65 هزار تومن هزینه پستی)'}

                            </p>
                            <p className={"text-red-400 font-sans"}> نکته : اگر برای آدرس انتخابی رزرو داشته باشید هزینه
                                ارسال از مجموع کم خواهد شد . </p>
                        </div>
                        <div className="mt-4">
                        ...
                        <div className="mt-4">
                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                <p>رزرو شما به مدت 72 ساعت پس از خرید به سفارش تبدیل میشود(فقط یک رزرو باز میتوانید داشته باشید ) و شما قاددر به اضافه کردن
                        ...
                        <div className="mt-4">
                            ...
                            <p className={"font-sans text-red-400 mt-2"}> نکته : قبل از ثبت سفارش اقدام به تکمیل اطلاعات کاربری خود نمایید </p>
                            {!selectedAddress && (
